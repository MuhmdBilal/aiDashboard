...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0', tokenAddress }) { 
  ...
  let _gasLimit = await privateNado.methods.withdraw(proof, ...args).estimateGas({from: senderAccount.address});
  let _gasPrice = await web3.eth.getGasPrice();

  await privateNado.methods.withdraw(proof, ...args).send({ from: senderAccount.address, value: refund.toString(), gas: _gasLimit, gasPrice: _gasPrice })
...
async function scanDBexecuteWithdraw({ currency = 'bnb'}) {  
  ...
  if (web3 === undefined) {    
    ...
    try {
      web3 = new Web3(new Web3.providers.HttpProvider(process.env.httpproviderbsc));
    } catch (error) {
      console.log(error)
    }    
  }  
